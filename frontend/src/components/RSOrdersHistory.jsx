...
function RSOrdersHistory() {
    ...
    if (confirm("Are you sure you want to delete this order?")) {
      try {
        const response = await fetch(
          `http://localhost:5002/restaurants/deleteOrder/${loggedInRestaurant._id}/${orderId}`,
          {
            method: "DELETE",
            credentials: "include",
          }
  ...
  async function handleDeleteOrderHistoryOfRestaurant() {
    if (confirm("Are you sure you want to delete all orders from your order history?")) {
      try {
        const response = await fetch(`http://localhost:5002/restaurants/deleteOrderHistory/${loggedInRestaurant._id}`, {
          method: "DELETE",
          credentials: "include",
        });
