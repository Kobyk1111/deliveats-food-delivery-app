...
function RSOrdersActive() {
      ...
      socket.on("orderStatusUpdated", ({ updatedOrderId, status, timestamp }) => {
        if (updatedOrderId) {
          ...
          setLoggedInRestaurant((prev) => {
            const updatedOrderHistory = prev.orderHistory.map((order) =>
              order.order._id === updatedOrderId ? { ...order, orderStatus: status, statusTimestamp: timestamp } : order
            );
            return { ...prev, orderHistory: updatedOrderHistory };
          });
        }
      });
  ...
  useEffect(() => {
    if (loggedInRestaurant?.orderHistory) {
      const statuses = {};
      const timestamps = {};
      loggedInRestaurant.orderHistory.forEach((eachOrder) => {
                  ...
                  <>
                    ...
                    <li>
                      <button
                        ...
                        disabled={currentStatusOrder >= getStatusOrder("Knock, knock! Your order is at the door. 🛎")}
                      >
                        Knock, knock! Your order is at the door. 🛎
                      </button>
                      {statusTimestamps[eachOrder.order._id] &&
                ...
                {eachOrder.order.additionalInfo?.orderType === "pickup" && (
                  <>
                    ...
                    <li>
                      ...
                      {statusTimestamps[eachOrder.order._id] &&
                        statusTimestamps[eachOrder.order._id]["You can now pick up your order at the counter 🏃"] && (
                          <p className="status-timestamp">
                            {statusTimestamps[eachOrder.order._id]["You can now pick up your order at the counter 🏃"]}
                          </p>
                        )}
                ...
                <li>
                  ...
                  {statusTimestamps[eachOrder.order._id] &&
                    statusTimestamps[eachOrder.order._id]["Delivery Completed"] && (
                      <p className="status-timestamp">{statusTimestamps[eachOrder.order._id]["Delivery Completed"]}</p>
                    )}
                </li>
              </ul>
            </div>
